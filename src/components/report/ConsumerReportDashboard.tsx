import { Activity, AlertTriangle, BarChart3, CheckCircle2, DollarSign, Globe2, Route, ShieldCheck, Target } from "lucide-react";
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <h1 className="font-display text-3xl font-bold text-foreground">{sanitizeConsumerText(inputs.projectName || "Feasibility Study")}</h1>
          <p className="mt-1 text-sm text-muted-foreground">{sanitizeConsumerText(inputs.industry || "Business concept")}{inputs.location ? ` · ${sanitizeConsumerText(inputs.location)}` : ""}</p>
        </div>
        <Badge className={`px-4 py-2 text-sm font-bold ${verdictTone(recommendation)}`}>{recommendation}</Badge>
      </div>
      ...
      <Tabs defaultValue="score" className="space-y-4">
        ...
        <TabsContent value="score" className="space-y-4">
          <div className="grid gap-3 md:grid-cols-2">{scoreRows.map(([label, scoreValue, finding]) => <ScoreRow key={label} label={label} score={scoreValue} finding={finding} />)}</div>
        </TabsContent>

        <TabsContent value="market" className="space-y-4">
          <div className="grid gap-3 md:grid-cols-3">
            <Kpi label="TAM" value={report.market.tamValue} sub={`${report.market.tamLabel} · CAGR ${report.market.tamCagr}`} />
            <Kpi label="SAM" value={report.market.samValue} sub={`${report.market.samLabel} · CAGR ${report.market.samCagr}`} />
            <Kpi label="SOM" value={report.market.somValue} sub={`${report.market.somLabel} · CAGR ${report.market.somCagr}`} />
          </div>
          <Card><CardHeader><CardTitle className="text-base">Competitive Positioning</CardTitle></CardHeader><CardContent className="grid gap-3 md:grid-cols-2">{report.competitors.slice(0, 8).map((c) => <div key={c.name} className="rounded-lg border border-border p-3"><div className="font-semibold text-foreground">{sanitizeConsumerText(c.name)}</div><p className="mt-1 text-xs text-muted-foreground">{sanitizeConsumerText(c.model)}</p><p className="mt-2 text-xs"><span className="font-semibold text-destructive">Gap:</span> {sanitizeConsumerText(c.weakness)}</p><p className="mt-1 text-xs"><span className="font-semibold text-primary">Wedge:</span> {sanitizeConsumerText(c.edge)}</p></div>)}</CardContent></Card>
        </TabsContent>
        ...
        <TabsContent value="risk" className="space-y-4">
          <Card><CardHeader><CardTitle className="text-base">Risk Register</CardTitle></CardHeader><CardContent className="grid gap-3 md:grid-cols-2">{report.risks.slice(0, 10).map((r) => <div key={r.name} className="rounded-lg border border-border p-3"><div className="flex items-start justify-between gap-3"><div className="font-semibold text-foreground">{sanitizeConsumerText(r.name)}</div><Badge variant="outline" className={riskTone(r.level)}>{sanitizeConsumerText(r.level)}</Badge></div><p className="mt-1 text-xs text-muted-foreground">Probability {sanitizeConsumerText(r.probability)} · Impact {sanitizeConsumerText(r.impact)}</p><p className="mt-2 text-sm text-foreground">{sanitizeConsumerText(r.mitigation)}</p></div>)}</CardContent></Card>
        </TabsContent>
        ...
        <TabsContent value="decision" className="space-y-4">
          <Card><CardHeader><CardTitle className="text-base">Strategic Recommendations</CardTitle></CardHeader><CardContent><ol className="space-y-2 text-sm">{recommendations.map((r, i) => <li key={`${r}-${i}`} className="flex gap-2"><span className="font-bold text-primary">{i + 1}.</span><span>{r}</span></li>)}</ol></CardContent></Card>
          <Card><CardHeader><CardTitle className="text-base">Final Decision</CardTitle></CardHeader><CardContent><p className="text-sm leading-7 text-foreground">Recommendation: <b>{recommendation}</b>. Use the next phase to validate the highest-impact assumptions before committing scale funding.</p></CardContent></Card>
        </TabsContent>
      </Tabs>
    </div>
  );
