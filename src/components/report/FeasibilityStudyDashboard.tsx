...
export function FeasibilityStudyDashboard({ report, inputs }: { report: FeasibilityReport; inputs: ConceptInputs }) {
  ...
  return <div className="space-y-8">
    ...
    <Tabs defaultValue="executive" className="space-y-5">
      ...
      <TabsContent value="score"><div className="grid gap-4 lg:grid-cols-2"><FMARTFigure report={report} /><Card><CardHeader><CardTitle>Score Drivers</CardTitle></CardHeader><CardContent><MiniTable head={["Dimension","Score","Finding"]} rows={[["Financial", report.scores.financial.toFixed(1), report.scores.financialFinding],["Market", report.scores.market.toFixed(1), report.scores.marketFinding],["Achievability", report.scores.achievability.toFixed(1), report.scores.achievabilityFinding],["Risk", report.scores.risk.toFixed(1), report.scores.riskFinding],["Timing", report.scores.timing.toFixed(1), report.scores.timingFinding],["Operational", report.scores.operational.toFixed(1), report.scores.operationalFinding]]} /></CardContent></Card></div></TabsContent>
      ...
      <TabsContent value="decision" className="space-y-4"><Card><CardHeader><CardTitle>Strategic Recommendations</CardTitle></CardHeader><CardContent><ol className="space-y-3">{sanitizeConsumerItems(report.recommendations, 8).map((r, i) => <li key={r} className="flex gap-3"><b className="text-primary">{i + 1}.</b><span>{r}</span></li>)}</ol></CardContent></Card><Card><CardHeader><CardTitle>Final Decision</CardTitle></CardHeader><CardContent><p>Recommendation: <b>{validation.recommendation}</b>. Use the next phase to validate the highest-impact assumptions before committing scale funding.</p></CardContent></Card></TabsContent>
    </Tabs>
  </div>;
}
