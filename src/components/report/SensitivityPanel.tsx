...
export const SensitivityPanel = ({ report }: { report: FeasibilityReport }) => {
  ...
  const { baseRev } = baseCase(report);

  const runSim = () => {
    setRunning(true);
    // Defer to next tick so UI updates
    setTimeout(() => {
      const result = runMonteCarlo(report, s, 2000);
      setMc(result);
      setRunning(false);
    }, 50);
  };
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Adjust drivers and run Monte Carlo to stress-test the base case.</p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-[1.1fr_1fr]">
        <Card>
          ...
          <CardContent className="space-y-5">
            <SliderRow label="Revenue per unit" sub={`Base ${fmt(baseRev)} / yr`} value={s.revenueMultiplier} onChange={(v) => setS({ ...s, revenueMultiplier: v })} min={0.5} max={1.5} step={0.05} format={pct} />
            ...
            <SliderRow label="Customer acquisition cost" sub="Higher = more expensive growth" value={s.cacMultiplier} onChange={(v) => setS({ ...s, cacMultiplier: v })} min={0.5} max={2} step={0.05} format={pct} />
            <SliderRow label="Conversion rate" sub="Funnel efficiency" value={s.conversionMultiplier} onChange={(v) => setS({ ...s, conversionMultiplier: v })} min={0.5} max={1.5} step={0.05} format={pct} />
            <SliderRow label="Market adoption" sub="SOM penetration speed" value={s.marketAdoptionMultiplier} onChange={(v) => setS({ ...s, marketAdoptionMultiplier: v })} min={0.5} max={1.5} step={0.05} format={pct} />
          </CardContent>
        </Card>
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            {[
              { label: "Revenue", value: fmt(outcome.revenue), delta: outcome.revenue - baseOutcome.revenue },
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-base flex items-center gap-2">
              <TrendingUp className="h-4 w-4 text-primary" /> Monte Carlo — {mc.iterations.toLocaleString()} trials
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
              <Card><CardContent className="p-4">
                <div className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Success probability</div>
                <div className="mt-1 font-display text-2xl font-bold text-success">{mc.successProbability.toFixed(0)}%</div>
                <div className="text-[11px] text-muted-foreground">trials with positive Y1 profit</div>
              </CardContent></Card>
              <Card><CardContent className="p-4">
