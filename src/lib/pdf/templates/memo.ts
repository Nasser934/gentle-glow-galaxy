...
export function placeExecutiveMemo(doc: Doc, sections: MemoSections) {
  paragraph(
    doc,
    "Read in under 60 seconds. Each block answers a single executive question. Bullets are intentionally short.",
  ...
  const block = (label: string, items: string[]) => {
    ...
    doc.y += 4; // extra breathing room between memo blocks
  };
