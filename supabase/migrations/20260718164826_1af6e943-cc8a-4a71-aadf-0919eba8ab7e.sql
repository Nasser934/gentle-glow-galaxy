...
returns text language sql volatile security definer set search_path = pg_catalog, extensions
...
select slug, id from public.reports where slug !~ '^[A-Za-z0-9_-]{20,64}$'
on conflict (old_slug) do nothing;

update public.reports set slug = public.generate_report_slug()
where slug !~ '^[A-Za-z0-9_-]{20,64}$';

alter table public.reports alter column slug set default public.generate_report_slug();
alter table public.reports drop constraint if exists reports_slug_url_safe;
alter table public.reports add constraint reports_slug_url_safe check (slug ~ '^[A-Za-z0-9_-]{20,64}$');
...
returns text language sql volatile security definer set search_path = pg_catalog, public
...
alter table public.reports add column if not exists display_id text;
update public.reports set display_id = public.generate_report_display_id() where display_id is null;
alter table public.reports alter column display_id set default public.generate_report_display_id(), alter column display_id set not null;
...
update public.reports set canonical_validated = coalesce(
  jsonb_typeof(output -> 'scores' -> 'overall') = 'number' and output ? 'scoringAudit' and output ? 'reportSchemaVersion',
  false
) where canonical_validated is null;

alter table public.reports alter column canonical_validated set default true, alter column canonical_validated set not null;

create or replace function public.enforce_report_canonical_validation()
returns trigger language plpgsql set search_path = pg_catalog as $$
begin
  if tg_op = 'INSERT' then new.canonical_validated := true;
  elsif new.output is distinct from old.output then new.canonical_validated := true;
  else new.canonical_validated := old.canonical_validated; end if;
  return new;
end; $$;
...
update public.reports set root_report_id = coalesce(parent_report_id, id) where root_report_id is null;

create or replace function public.set_report_root_id()
returns trigger language plpgsql security invoker set search_path = pg_catalog, public as $$
begin
  if new.parent_report_id is null then new.root_report_id := new.id;
  else
    select coalesce(r.root_report_id, r.id) into new.root_report_id from public.reports r where r.id = new.parent_report_id;
    if new.root_report_id is null then raise exception 'Parent report does not exist'; end if;
  end if;
  return new;
end; $$;
drop trigger if exists trg_reports_root_id on public.reports;
create trigger trg_reports_root_id before insert or update of parent_report_id on public.reports
...
create index if not exists analysis_requests_user_started_idx on public.analysis_requests(user_id, started_at desc);
create index if not exists analysis_requests_status_idx on public.analysis_requests(completion_status, started_at);
create index if not exists analysis_rate_limits_cleanup_idx on public.analysis_rate_limits(window_start);
...
grant select (id, function_name, started_at, completed_at, model_id, research_status, completion_status, failure_category) on public.analysis_requests to authenticated;
grant all on public.analysis_requests to service_role;
grant all on public.analysis_rate_limits to service_role;

drop policy if exists "Users view own safe analysis request metadata" on public.analysis_requests;
create policy "Users view own safe analysis request metadata"
on public.analysis_requests for select to authenticated using (auth.uid() = user_id);

create or replace function public.begin_analysis_request(
  p_function_name text, p_idempotency_key text, p_request_hash text, p_ip_hash text default null
...
declare
  ...
  v_user_hour integer; v_user_day integer; v_ip_hour integer := 0;
  v_user_hour_limit integer; v_user_day_limit integer; v_ip_hour_limit integer;
begin
  if v_user_id is null then return query select null::uuid, false, 'unauthenticated'::text, 0; return; end if;
  if coalesce(p_function_name, '') not in ('analyze-concept', 'autofill-brief', 'complete-field')
    ...
    return query select null::uuid, false, 'invalid_request_metadata'::text, 0; return;
  end if;
  select
  ...
  insert into public.analysis_requests (user_id, function_name, idempotency_key, request_hash, ip_hash)
  values (v_user_id, p_function_name, p_idempotency_key, p_request_hash, nullif(p_ip_hash, ''))
  on conflict (user_id, function_name, idempotency_key) do nothing
  returning id into v_request_id;

  if v_request_id is null then
    select ar.id into v_request_id from public.analysis_requests ar
    where ar.user_id = v_user_id and ar.function_name = p_function_name and ar.idempotency_key = p_idempotency_key;
    return query select v_request_id, false, 'duplicate_request'::text, 0; return;
  end if;
  ...
  if v_user_hour > v_user_hour_limit or v_user_day > v_user_day_limit or v_ip_hour > v_ip_hour_limit then
    update public.analysis_requests set completion_status = 'rate_limited', completed_at = now(), failure_category = 'usage_limit'
    where id = v_request_id;
    return query select v_request_id, false, 'usage_limit'::text, greatest(1, 3600 - extract(epoch from (now() - v_hour))::integer); return;
  end if;
...
create or replace function public.complete_analysis_request(
  p_request_id uuid, p_completion_status text, p_model_id text default null,
  p_prompt_version text default null, p_usage_metadata jsonb default '{}'::jsonb,
  p_research_status text default 'not_requested', p_failure_category text default null
) returns boolean language plpgsql security definer set search_path = pg_catalog, public as $$
declare v_updated integer;
begin
  if p_completion_status not in ('completed', 'failed')
     or p_research_status not in ('complete', 'partial', 'failed', 'not_requested') then return false; end if;
  update public.analysis_requests
  set completed_at = now(), completion_status = p_completion_status,
      model_id = left(p_model_id, 120), prompt_version = left(p_prompt_version, 120),
      usage_metadata = jsonb_strip_nulls(jsonb_build_object(
        'prompt_tokens', case when coalesce(p_usage_metadata ->> 'prompt_tokens', '') ~ '^[0-9]{1,12}$' then to_jsonb((p_usage_metadata ->> 'prompt_tokens')::bigint) else null end,
        'completion_tokens', case when coalesce(p_usage_metadata ->> 'completion_tokens', '') ~ '^[0-9]{1,12}$' then to_jsonb((p_usage_metadata ->> 'completion_tokens')::bigint) else null end,
        'total_tokens', case when coalesce(p_usage_metadata ->> 'total_tokens', '') ~ '^[0-9]{1,12}$' then to_jsonb((p_usage_metadata ->> 'total_tokens')::bigint) else null end
      )),
      research_status = p_research_status, failure_category = left(p_failure_category, 80)
  where id = p_request_id and user_id = auth.uid() and completion_status = 'running';
